...
@section('title','Display Property Category')
@section('page_title','All Property Category')
@section('body')
        ...
        <form action="{{ route('admin.property.category.store') }}" method="post">
            ...
            <div class="modal-body">
                <div class="row">
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            <label for="" class="col-form-label form-label">Listing Template <span
                                    class="text-danger">*</span></label>
                            <select name="page_template" id="" class="form-select">
                                <option value="default">Default</option>
                                @foreach(get_templates() as $path=>$pageTemplate)
                                <option value="{{ $path }}">{{ $pageTemplate }}</option>
                                @endforeach
                            </select>
                            @error('page_template')
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            <label for="" class="col-form-label form-label">Listing Template <span
                                    class="text-danger">*</span></label>
                            <select name="page_template" id="" class="form-select">
                                <option value="default" {{ ($edit_data->page_template == 'default')? 'selected':'' }}>Default</option>
                                @foreach(get_templates() as $path=>$pageTemplate)
                                <option value="{{ $path }}" {{ ($edit_data->page_template == $path)? 'selected':'' }}>{{ $pageTemplate }}</option>
                                @endforeach
                            </select>
                            @error('page_template')
                                <p class="text-danger">{{ $message }}</p>
