...
if(request()->get('landscaping_id') !=NULL){
 $pr = get_landscaping(request()->get('landscaping_id'));
 $totalAmount += $pr->price;
...
<div class="quote-section">
    <div class="container">
      ...
      <div class="row type-row">
        <div class="col-12 col-sm-12">
          <h6>Step 06</h6>
          <h1>Select your landscaping package</h1>
        </div>
        <div class="col-12 col-sm-12 package-item owl-carousel">
          @foreach($landscaping as $data)
          <a href="{{ $url }}&landscaping_id={{ $data->id }}">
            <div class="package-box">
              <h3>{{ $data->title }} <i class="fa fa-check"></i></h3>
              <h4>$ {{ $data->price }}</h4>
    ...
    <div class="container">
      <div class="row">
        <div class="col-12 col-sm-12 col-md-3 col-lg-3">
          <a href="{{ route('home.build-quits.step_five') }}{{ $back }}" class="btn btn-back">Back</a>
        </div>
        <div class="col-12 col-sm-12 col-md-6 col-lg-6">
          <h6>Sub-total <span>${{ $totalAmount }}</span></h6>
        </div>
        <div class="col-12 col-sm-12 col-md-3 col-lg-3">
          <a href="{{ route('home.build-quits.step_seven') }}{{ $param }}" class="btn btn-next">Next</a>
        </div>
      </div>
    </div>
  </div>
